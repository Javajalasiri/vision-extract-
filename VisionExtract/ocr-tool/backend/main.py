import os, re, json, base64, uuid, shutil, io
...
# Strip ALL whitespace from key to prevent "Illegal header value" error
...
Each record must have exactly these keys:
{"file_row":"FILENAME-N","id_code":"...","primary_name":"...","location":"City,State","loan_amount":"digits only","interest_pct":"23%","years":"16","rate_pct":"3.64%","pv_reduction":"21.87%","mp_reduction":"7.76%","ti_reduction":"18.87%","secondary_name":"...","secondary_code":"..."}
Rules: Convert word-numbers to digits. Keep codes exactly as seen. Return ONLY the JSON array."""

def compress_image(img_path: Path) -> str:
    """Resize + compress image to JPEG base64 to avoid header size issues."""
...
async def extract_with_openrouter(img_path: Path, filename: str) -> list:
    if not OPENROUTER_API_KEY:
        raise ValueError("OPENROUTER_API_KEY is not set in environment variables")

    b64 = compress_image(img_path)
    auth_header = "Bearer " + OPENROUTER_API_KEY  # explicit concat avoids f-string whitespace bugs
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        resp = await client.post(
            ...
            headers={
                ...
                "HTTP-Referer": "https://visionextract.onrender.com",
                "X-Title": "VisionExtract"
            },
            json={
                "model": MODEL,
                "max_tokens": 4000,
...
@app.get("/")
def home():
    key_status = "SET" if OPENROUTER_API_KEY else "MISSING"
    return {"status": "OCR Tool Running 🚀", "model": MODEL, "api_key": key_status}
